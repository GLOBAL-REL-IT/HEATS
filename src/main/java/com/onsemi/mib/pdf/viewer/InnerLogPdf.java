...
public class InnerLogPdf extends AbstractITextPdfViewPotrait {
...
//        PageSize.A4.rotate();
        ...
        String title = "RMS LOT INFORMATION\n\n";
        ...
        List<LogRmsLot> logDetailsList = (List<LogRmsLot>) model.get("logDetailsList");

        boolean flag = false;
        int i = 0;
        System.out.println("logDetailsList.size() : " + logDetailsList.size());
        while(i<logDetailsList.size()) {
            if(i==0) {
                cellHeader.setPhrase(new Phrase("Material Pass No.", fontHeader));
                table.addCell(cellHeader);
                cellContent.setPhrase(new Phrase(logDetailsList.get(i).getRmsId(), fontContent));
                table.addCell(cellContent);

                //2
                cellHeader.setPhrase(new Phrase("Material Pass Expiry Date", fontHeader));
                table.addCell(cellHeader);
                cellContent.setPhrase(new Phrase(logDetailsList.get(i).getRmsEvent(), fontContent));
                table.addCell(cellContent);
            }
            doc.add(table);
        }
...
//                
//                //8
//                cellHeader.setPhrase(new Phrase("Received Date", fontHeader));
//                table.addCell(cellHeader);
